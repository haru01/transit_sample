...
class RouteMap
  ...
  route: (from, to, visitedStations=[]) ->
    ...
    nextStations = @otherStationsExcudeVisited(from, visitedStations)

    return @newRoute(visitedStations, to) if _.include(nextStations, to)

    for next in nextStations
      route = @route(next, to, _.clone(visitedStations))
      return route if (route)

    return null

  otherStationsExcudeVisited: (station, visitedStations) ->
    (link.other(station) for link in @links when !_.include(visitedStations, link.other(station)) and link.include(station))
...
class Link
  ...
  isLink: (stationA, stationB) ->
    (@stationA == stationA and @stationB == stationB) or
    (@stationB == stationA and @stationA == stationB)

  include: (station) ->
    (@stationA == station or @stationB == station)
...
standardMap = ()-> new RouteMap((map) ->
  map.addLink(row[0], row[1], row[2]) for row in [  ['横浜', 'XXX', 14],
                                                  ['横浜', '川崎', 14],
                                                  ['川崎', '東京', 24],
                                                  ['東京', '秋葉原', 6],
                                                  ['秋葉原', '田端', 11],
